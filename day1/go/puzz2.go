// Same as previous puzzle, but find 3 numbers that sum to 2020 and then get product
  ...
  for i := 0; i < len(nums); i++ {
    for j := 0; j < len(nums); j++ {
			...
			for k := 0; k < len(nums); k++ {
				...
				if sum := nums[i] + nums[j] + nums[k]; sum == 2020 {
					fmt.Println(nums[i], nums[j], nums[k], "product is", nums[i] * nums[j] * nums[k])
					break Loop
				}
			}
		}
