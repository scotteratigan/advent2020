...
func main() {
  ...
  for i := 0; i < len(nums); i++ {
    for j := 0; j < len(nums); j++ {
			...
			if sum := nums[i] + nums[j]; sum == 2020 {
				fmt.Println(nums[i], nums[j], "product is", nums[i] * nums[j])
				break Loop
			}
		}
  }
...
// The above works fine but it re-checks lots of numbers potentially.